...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.3) #test_size - w jakim stosunu dzielone sa dane, 30% dla danych testowych, 70% dla trenowanych
model = DecisionTreeClassifier(criterion='entropy', max_depth=1) #uzyta entropia do znalezienia najlepszego atrybutu
AdaBoost = AdaBoostClassifier(base_estimator=model, n_estimators=400, learning_rate=1) #learning rate default as 1, 
